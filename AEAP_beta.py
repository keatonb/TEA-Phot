...
USAGE:
    ...
    - OPTIONS:
        python AEAP_beta.py <observatory> <instrument> <image.fits>
            ...
            --do_clip <10> --extinction <0.25> --image_dir <./>
            --bias_dir <./> --flat_dir <./> --out_dir <./>
...
def image_reduction(instrument, bias, bias_dir, flat, flat_dir, do_plot,
    ...
    if instrument == "SHOC":
        ...
        [The code can be modified here to include a meadian DARK subtraction if
        needed]
        """

        # FLAT: combine and normalise by median of FLAT frames
    ...
    # STE INSTRUMENT
        ...
        if len(bias_files) == 0:
            print("     !!! ERROR: no BIAS files containing the string {}"
        ...
        for i in bias_files:
            ...
            if bias_header['OBJECT'] not in ["BIAS"]:
                print("     ! WARNING: are you sure {} is a BIAS"
                      " file? \n".format(i))
                continue
                ...
                trim_x1, trim_x2 = new_trim[0], new_trim[1]
                trim_y1, trim_y2 = new_trim[2], new_trim[3]
                # s = header['BIASSEC'] # "bias overscan region"
                bias_data = bias_hdu.data.astype(np.float)
                # bias_data = bias_data[int(trim_y1):int(trim_y2), int(trim_x1):int(trim_x2)]  # removed (no need to trim STE image)
        ...
        """
        # DARKS: combine and take median of DARK frames. [The code can be
        modified here to include a meadian DARK subtraction if needed]
        """

        # FLATS: combine and normalise by median of FLAT frames
        flat_files = []
        flat_concat = []
        ...
        if len(flat_files) == 0:
            print("     !!! ERROR: no FLAT files containing the string {}"
                  " found in {} \n".format(flat, flat_dir))
            exit()
        ...
        for i in flat_files:
            ...
            if flat_header['OBJECT'] not in ["SKYFLAT"]:
                print("     ! WARNING: are you sure {} is a FLAT"
                      " file?".format(i))
                continue
                ...
                # flat_data = flat_data[int(trim_y1):int(trim_y2), int(trim_x1):int(trim_x2)]  # removed (no need to trim STE image)
...
def get_observatory(observatory, **kwargs):
    """
    If you want a *precise* location of a telescope, you can define it below as
    an if statement
    """
    if observatory == "SAAO":
...
def find_midpoint(instrument, header, iter):
    ...
    if instrument == "SHOC":
        ...
        # External trigger: FRAME timestamp is *START* of the first exposure
            ...
            exp_time = np.float(header['GPS-INT'])     # units of milli-sec
...
def find_instrument(instrument, header):
    ...
    if instrument == "SHOC":
        ...
        preamp_gain = np.str(header['PREAMP'])      # used to look-up sensitivity gain
...
def write_new_fits(instrument, hdu, red_image, fit_path, star_name,
    ...
    if instrument == "SHOC":
        ...
        if trigger == "Internal":
            ...
            new_hdu.header['EXPOSURE'] = (str(hdu.header['EXPOSURE']), "exposure time (sec)")
            new_hdu.header['READTIME'] = (str(read_time*86400.), "CCD read out time (sec)")
        elif trigger == "External":
        ...
        new_hdu.header['JD-MID'] = (str(midpoint_JD-2450000.), "midpoint of exposure (JD-2450000.0)")
...
if __name__ == '__main__':
    ...
    # set up Python 2.x to Python 3.x fixes
    if sys.version_info[0] < 3:
        try:
            input = raw_input
        except ImportError:
            print("\n ! WARNING: looks like you are using this Python 3.x"
                  " script with a Python 2.x interpreter. It will work but"
                  " you could consider upgrading to Python 3.x to avoid"
    ...
    parser.add_argument('observatory', type=str,
                        help="Observatory name (e.g. SAAO); <str>")
    parser.add_argument('instrument', type=str,
                        help="Instrument name (e.g. SHOC or STE); <str>")
    parser.add_argument('image', type=str,
                        help="If using the SHOC instrument <image> is the filename of image FITS cube; if using the STE instrument <image> is automatically set to 'a' <str>")
    parser.add_argument('--bias', type=str,
                        help="Filename of bias FITS cube (with SHOC), or a common pre-fix string to search for multiple bias files (e.g. STE); <str>")
    ...
    parser.add_argument('--flat', type=str,
                        help="Filename of flat FITS cube (with SHOC), or a or a common pre-fix string to search for multiple flat files (e.g. STE); <str>")
    parser.add_argument('--image_dir', type=str, default='./', action='store',
                        help="Directory path of where image file(s) are located; <str>")
    parser.add_argument('--bias_dir', type=str, default='./', action='store',
                        help="Directory path of where bias file(s) are located; <str>")
    ...
    parser.add_argument('--flat_dir', type=str, default='./', action='store',
                        help="Directory path of where flat file(s) are located; <str>")
    parser.add_argument('--out_dir', type=str, default='./', action='store',
                        help="Directory path of where code output; <str>")
    parser.add_argument('--object', default=None, type=str,
                        help="Overwrite the object name in FITS header; <str>")
    parser.add_argument('--coord', default=None, type=two_strs,
                        help="Overwrite the coordinates [ra,dec] of target star in FITS header; <'00:00:00.00 -00:00:00.00'> ")
    parser.add_argument('--source_sigma', default='10', action='store',
                        type=str2flt, help="The S/N of the image source extraction; <float>")
    parser.add_argument('--do_plot', default=False, type=str2bool,
                        help="Plot and save all figures; <bool>")
    parser.add_argument('--extract', default=False, type=str2bool,
                        help="Extract and save individual FITS files containing reduced images in a subdirectory; <bool>")
    parser.add_argument('--do_clip', default=None, type=str2flt,
                        help="Sigma value for outlier removal in differential light curve; <float>")
    parser.add_argument('--extinction', default=None, type=str2flt,
                        help="Include extinction correction to individual light curves (units of magnitude); <float>")
    ...
    """
    # TIP: you will need to include alternative instrument names below, but
    also at all other necessary locations (and functions) in the code
    """
    known_instruments = ["SHOC", "STE"]

    if instrument == "SHOC":
        cube = True  # SHOC provides FITS cubes as output
    elif instrument == "STE":
        """
        ...
        cube = False  # STE provides individual FITS files as output
    ...
    if not os.path.exists(bias_dir):
        print("\n     !!! ERROR: specified BIAS directory does not"
              " exist: {} \n".format(bias_dir))
        exit()
    """
    ...
    if not os.path.exists(flat_dir):
        print("\n     !!! ERROR: specified FLAT directory does not"
              " exist: {} \n".format(flat_dir))
        exit()
    ...
    if not os.path.exists(out_dir):
        print("\n (*) The specfied out_dir = {} has been created since"
              " it did not exist".format(out_dir))
        os.makedirs(out_dir)
    out_dir = out_dir+"/"

    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    # BELOW SETS UP TO CUBE MODULE (E.G. NEEDED FOR SHOC AT SAAO)
    """
    ...
    if cube:
        ...
        if os.path.isfile(image_dir+'/'+image) is False:
            print("\n     !!! ERROR: specified IMAGE file does not exist at:"
                  " {}/{} \n".format(image_dir, image))
            exit()
        ...
        if args.bias:
            ...
            if os.path.isfile(bias_dir+'/'+bias) is False:
                print("\n     !!! ERROR: specified BIAS file does not exist"
                      "  at: {}/{} \n".format(bias_dir, bias))
                exit()
        ...
        if args.dark:
            dark = args.dark
            if os.path.isfile(dark_dir+'/'+dark) is False:
                print("\n     !!! ERROR: specified DARK file does not exist"
                      "  at: {}/{} \n".format(dark_dir, dark))
                exit()
        """
        ...
        if args.flat:
            ...
            if os.path.isfile(flat_dir+'/'+flat) is False:
                print("\n     !!! ERROR: specified FLAT file does not exist"
                      "  at: {}/{} \n".format(flat_dir, flat))
                exit()
    ...
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    # BELOW SETS UP TO NON-CUBE MODULE (E.G. NEEDED FOR STE AT SAAO)

    if cube is False:

        # search for IMAGE files in specified image_dir
        image_files = []
    ...
    if args.object:
        ...
        print("     > Using user-defined name of object: {}".format(star_name))
    else:
        star_name = find_object_name(instrument, header)
        print("     > Using name of object from FITS header:"
              " {}".format(star_name))
    ...
    if args.coord:
        ...
        print("     > Using user-defined RA and DEC coordinates: \n"
              "       {} and {}".format(star_ra, star_dec))
    else:
        star_ra, star_dec = find_object_coord(instrument, header)
        print("     > Using the RA and DEC from FITS header: \n"
              "       {} and {} \n"
              "\n     ! WARNING: is the contents of the FITS header correct?"
              "\n     It is advised to define the name, RA and DEC at the"
              " command line using: \n"
              "     --object <HD123456> --coord <'00:00:00 -00:00:00'>"
              "\n".format(star_ra, star_dec))

    # define S/N for source extraction from command line (default of 10)
    if args.source_sigma:
        print("     > Using source extraction S/N criterion of:"
              " {}".format(source_sigma))
    ...
    if args.flat and args.bias:
        ...
        print("\n     !!! ERROR: it appears that both BIAS and FLAT files have"
              "\n     not been provided at the command line. Both are needed"
              "\n     if image reduction is required")
        exit()
    ...
    is 'External' or 'External Start' when using SHOC. In other cases, the loop
    of image files starts at a counter of zero
    """
    if instrument == "SHOC":
        n_start = 2
    elif instrument == "STE":
    ...
    for i in range(n_start, n_slice):
        ...
        if instrument == "STE":
            trim = header['TRIMSEC']  # the 'useful' part of the data
            new_trim = re.findall(r'\d+', trim)
            trim_x1, trim_x2 = new_trim[0], new_trim[1]
            trim_y1, trim_y2 = new_trim[2], new_trim[3]
            # s = header['BIASSEC'] # "bias overscan region"
        else:
            trim_x1, trim_x2 = 0, hdu.data.shape[0]
            trim_y1, trim_y2 = 0, hdu.data.shape[1]

        raw_image = hdu.data.astype(dtype=float)
        # raw_image = raw_image[int(trim_y1):int(trim_y2), int(trim_x1):int(trim_x2)]  # removed (no needed to trim STE image)

        # create an image mask for source extraction
        """
        NOTE: masks work in sep.extract() such that flux within 'True' pixels
        is not included in aperture photometry
        """
        ...
        midpoint_BJD = np.float(midpoint_BJD.value) - 2450000.
        ...
        if i == n_start:
            ...
            # plt.title("CCD image of "+str(star_name))  # removed
            ...
            while True:
                try:
                    ...
                    if toolbar.mode == '':
                        print("\n     >>> Click the image to choose the"
                              " COMPARISON star's co-ordinates:")
                    click = fig3.ginput(1, timeout=0, show_clicks=False)
                    comp_pix_x = list(map(lambda x: x[0], click))
                    comp_pix_y = list(map(lambda x: x[1], click))
                    ...
                    print("\n     !!! ERROR: you need to click on the screen"
                          "to choose a comparison star")
                    continue

                # allow the user to choose the comparison as the target
                if (objects['x'][index_comp] == objects['x'][index]
                        and objects['y'][index_comp] == objects['y'][index]):
                    print("\n     ! WARNING: you have chosen the same"
                          " source as target and comparison stars \n"
                          "\n     >>> Proceed with the target star as the"
                          " comparison star?")
                    CeqT = str((input("     (y/n) = ")))
                    while CeqT not in (Y_proceed+N_proceed+Q_proceed):
                        print("\n     !!! ERROR: you need to type 'y' to"
                              " proceed or 'n' to exit \n"
                              "     >>> Proceed with the target star as the"
                              " comparison star?")
                        CeqT = str((input("     (y/n) = ")))
                    if CeqT in (N_proceed+Q_proceed):
                        print("\n     > You have chosen not to use the"
                              " target star as the comparison star")
                        exit()
                    if CeqT in (Y_proceed):
                        print("\n     ! WARNING: differential light curve will"
                              " be zero as target is the comparison")
                        break

                # break loop if target found successfully
                if index_comp.any() == True:  # MUST be '==' and not 'is'
                    tmp_x = ("{:.2f}".format(float(objects['x'][index_comp])))
                    tmp_y = ("{:.2f}".format(float(objects['y'][index_comp])))
                    ...
                    toolbar = plt.get_current_fig_manager().toolbar
                    if toolbar.mode == '':
                        print("\n     !!! ERROR: no comparison object found"
            ...
            sky_background = ""
            print("\n     >>> Calculate the sky background using annuli or a"
                  " 'sky star'?")
            sky_background = str(input("     (annuli/star) = "))

            while sky_background not in ("annuli", "star", "quit"):
                print("\n !!! ERROR: you need to type 'annuli' or 'star' to"
                      " proceed or 'quit' to exit \n")
                print("     >>> Calculate the sky background suing annuli or a"
                      " 'sky star'?")
                sky_background = str(input("     (annuli/star) = "))
            ...
            if sky_background == "star":
                ...
                while True:
                    try:
                        ...
                        if toolbar.mode == '':
                            print("\n     >>> Click the image to choose the"
                                  " sky-star background co-ordinates:")
                        click = fig3.ginput(1, timeout=0, show_clicks=False)
                        sky_pix_x = list(map(lambda x: x[0], click))
                        sky_pix_y = list(map(lambda x: x[1], click))

                    except ValueError:
                        print("\n     !!! ERROR: you need to click on the"
                              " screen to choose the sky-star background"
                              " location")
                        continue
                    ...
                    if index_sky.any() == True:  # MUST be '==' and not 'is'
                        print("\n     !!! ERROR: sky background aperture is"
                              " too close to an object \n")
                        continue
                    else:
                        sky_pix_x = sky_pix_x[0]
                        sky_pix_y = sky_pix_y[0]
                        break
                ...
                tmp_sky_x = ("{:.2f}".format(float(sky_pix_x)))
                tmp_sky_y = ("{:.2f}".format(float(sky_pix_y)))
                print("     > Sky-Star background located at: \n"
                      "     > (X,Y) = ({}, {}) \n".format(tmp_sky_x, tmp_sky_y))
                print("     >>> Are you happy with your choices of: \n"
                      "         target (green) \n"
                      "         comparison (blue) \n"
                      "         sky background (yellow)?")
                happy = str(input("         (y/n) = "))

                while happy not in (Y_proceed+N_proceed+Q_proceed):
                    print("\n     !!! ERROR: you need to type y to proceed"
                          " or n to exit \n")
                    print("     >>> Are you happy with your choices of: \n"
                          "         target (green) \n"
                          "         comparison (blue) \n"
                ...
                if happy in (N_proceed+Q_proceed):
                    print("\n     > You have specified that incorrect"
                          " objects have been chosen: exiting program \n")
                    exit()
        ...
        # for all remaining frames (after the 'first'), find each star's
        ...
        if len(objects[index]) < 1:
            print("     ! WARNING: no target star detected in frame"
                  " {}".format(i))
            do_targ = False

        if len(objects[index_comp]) < 1:
            print("     ! WARNING: no comparison star detected in frame"
                  " {}".format(i))
            do_comp = False

        # skip frame if multiple apertures are fitted
        if len(objects[index]) > 1:
            print("     ! WARNING: multiple apertures on target star in"
                  " frame {}".format(i))
            do_targ = False

        if len(objects[index_comp]) > 1:
            print("     ! WARNING: multiple apertures on comparison star "
                  " in frame {}".format(i))
            do_comp = False

        if do_targ:
            # warn user if CCD exceeds linearity (not really needed)
            tmp_flux = int(objects['peak'][index])
            if (tmp_flux > 58000 and tmp_flux < 62000):
                print("     ! WARNING: target star flux is high: {} counts"
                      " in frame {} \n".format(tmp_flux, i))
            elif (tmp_flux >= 62000):
                print("     ! WARNING: target star flux has saturated: {}"
                      " counts in frame {} \n".format(tmp_flux, i))
                do_targ = False

        if do_comp:
            # skip frame if CCD saturates
            tmp_flux = int(objects['peak'][index_comp])
            if (tmp_flux > 58000 and tmp_flux < 62000):
                print("     ! WARNING: comparison star flux is high: {} counts"
                      " in frame {} \n".format(tmp_flux, i))
            elif (tmp_flux >= 62000):
                print("     ! WARNING: comparison star flux has saturated: {}"
                      " counts in frame {} \n".format(tmp_flux, i))
                do_comp = False

        # if both target and comparison stars get errors, proceed to next frame
        if do_targ is False and do_comp is False:
            print("     ! WARNING: neither the target nor the comparison"
                  " star were extracted in frame: {}"
                  " (skipping frame) \n".format(i))
            continue
        ...
        if i == n_start:
            ...
            # calculate curve of growth for target and comparison stars
            for k in ap_size:

                flux, fluxerr, \
                    flag = sep.sum_ellipse(red_image, objects['x'][index], objects['y'][index],
                                           int(k)*objects['a'][index], int(k)*objects['b'][index],
                                           objects['theta'][index], r=1., err=bkg.globalrms,
                                           gain=gain, mask=None)

                cg_flux.extend(flux)
                cg_area.extend(np.pi * int(k) * objects['a'][index] * int(k) * objects['b'][index])

                flux_comp, fluxerr_comp, \
                    flag_comp = sep.sum_ellipse(red_image, objects['x'][index_comp], objects['y'][index_comp],
                                                int(k)*objects['a'][index_comp], int(k)*objects['b'][index_comp],
                                                objects['theta'][index_comp], r=1., err=bkg.globalrms,
                                                gain=gain, mask=None)
            ...
            grid = plt.GridSpec(10, 10, hspace=0., wspace=0.)
            ...
            for m1, m2, m3 in zip(cg_area_comp[1:], cg_flux_comp[1:],
                                  ap_size[1:]):
                ax2.text(m1, m2, str(m3), fontsize=10, color='w', va='center',
                         ha='center', zorder=3)

            # plt.title("Curve of Growth for varying aperture sizes")  # removed
            plt.show(block=False)
            plt.pause(0.1)
            ...
            while True:
                try:
                    print("\n     >>> Choose the best aperture size for the"
                          " (green) target star")
                    print("         (i.e. where the curve of growth becomes"
                          " linear)")
                    cg_input = int(input("         (int) = "))
                except ValueError:
                    print("\n     !!! ERROR: you need to input an INTEGER"
                          " value \n")
                    continue

                if (cg_input < np.min(ap_size) or cg_input > np.max(ap_size)):
                    print("\n     !!! ERROR: you need to choose an integer"
                          "aperture from these plotted \n")
                    continue
                else:
                    break
            ...
            while True:
                try:
                    print("\n     >>> Choose the best aperture size for the"
                          " (blue) comparison star")
                    print("         (i.e. where the curve of growth becomes"
                          " linear)")
                    cg_input_comp = int(input("         (int) = "))
                except ValueError:
                    print("\n     !!! ERROR: you need to input an INTEGER"
                          " value \n")
                    continue
            ...
            tmp_a = ("{:.2f}".format(float(opt_ap_size*objects['a'][index])))
            tmp_b = ("{:.2f}".format(float(opt_ap_size*objects['b'][index])))
            print("\n     > optimum aperture parameters for target:")
            print("       semi-major and minor axes = "
                  "[ {}, {} ] (pixels)".format(tmp_a, tmp_b))

            tmp_a = ("{:.2f}".format(float(opt_ap_size*objects['a'][index_comp])))
            tmp_b = ("{:.2f}".format(float(opt_ap_size*objects['b'][index_comp])))
            print("\n     > optimum aperture parameters for comparison:")
            print("       semi-major and minor axes = "
                  "[ {}, {} ] (pixels)".format(tmp_a, tmp_b))

            # output to screen so user is aware to be patient
            ...
            if do_plot:
                ...
                if sky_background == "star":
                    ...
                    # overplot objects from source extraction <<< TMP HERE <<<
                    tmp_objects = np.copy(objects)
                    tmp_objects['x'][index], tmp_objects['y'][index] = -99, -99
                    tmp_objects['x'][index_comp], tmp_objects['y'][index_comp] = -99, -99
                    for j in range(len(tmp_objects)):
                        ellipse_plot(tmp_objects['x'][j], tmp_objects['y'][j], size,
                                     tmp_objects['a'][j], tmp_objects['b'][j],
                                     tmp_objects['theta'][j], 'none', 'red', [1])

                    # target ellipse
                    ellipse_plot(objects['x'][index], objects['y'][index],
                                 opt_ap_size, objects['a'][index],
                                 objects['b'][index], objects['theta'][index],
                                 'none', 'green', [1., 1.5, 2.0])  # <<< TMP HERE <<<
        ...
        if sky_background == "star":
            try:
                ...
                if do_targ:
                    flux, fluxerr, \
                        flag = sep.sum_ellipse(red_image, objects['x'][index],
                                               objects['y'][index],
                                               opt_ap_size*objects['a'][index],
                                               opt_ap_size*objects['b'][index],
                                               objects['theta'][index], r=1.,
                    ...
                    flux_sky, fluxerr_sky, \
                        flag_sky = sep.sum_ellipse(red_image, sky_pix_x,
                                                   sky_pix_y,
                                                   opt_ap_size*objects['a'][index],
                                                   opt_ap_size*objects['b'][index],
                                                   objects['theta'][index],
                                                   r=1., err=bkg.globalrms,
                                                   gain=gain,
                                                   mask=image_mask_all)
                    ...
                    flux = flux - flux_sky
                    fluxerr = np.sqrt((fluxerr**2.) + (fluxerr_sky**2.))
                ...
                if do_comp:
                    flux_comp, fluxerr_comp, \
                        flag_comp = sep.sum_ellipse(red_image,
                                                    ...
                                                    r=1., err=bkg.globalrms,
                                                    gain=gain,
                                                    mask=image_mask_comp)

                    flux_comp_sky, fluxerr_comp_sky, \
                        flag_comp_sky = sep.sum_ellipse(red_image, sky_pix_x,
                                                        sky_pix_y,
                                                        opt_ap_size_comp*objects['a'][index_comp],
                                                        opt_ap_size_comp*objects['b'][index_comp],
                                                        objects['theta'][index_comp],
                    ...
                    fluxerr_comp = np.sqrt((fluxerr_comp**2.)
                                           + (fluxerr_comp_sky**2.))
                ...
                print("     !!! ERROR: ZeroDivisionError in sep.sum_ellipse()"
                      " when using sky-star")
                print("                Your chosen aperture sizes may be"
                      " larger than the CCD image")
                continue
        ...
        if sky_background == "annuli":
            try:
                if do_targ:
                    flux, fluxerr, \
                        flag = sep.sum_ellipse(red_image,
                                               objects['x'][index], objects['y'][index],
                                               opt_ap_size*objects['a'][index],
                                               opt_ap_size*objects['b'][index],
                                               objects['theta'][index],
                                               r=1., err=bkg.globalrms,
                                               gain=gain, mask=image_mask_targ,
                                               bkgann=(1.5*opt_ap_size*objects['a'][index],
                                                       2.*opt_ap_size*objects['a'][index]))
                ...
                if do_comp:
                    flux_comp, fluxerr_comp, \
                        flag_comp = sep.sum_ellipse(red_image,
                                                    objects['x'][index_comp], objects['y'][index_comp],
                                                    opt_ap_size*objects['a'][index_comp],
                                                    opt_ap_size*objects['b'][index_comp],
                                                    objects['theta'][index_comp],
                                                    r=1., err=bkg.globalrms, gain=gain,
                                                    mask=image_mask_comp,
                                                    bkgann=(1.5*opt_ap_size*objects['a'][index_comp],
                                                            2.*opt_ap_size*objects['a'][index_comp]))
        ...
        change_pix_x = (init_pix_x - 0.5*(x_min_pix_objects[index]
                        + x_max_pix_objects[index]))
        change_pix_y = (init_pix_y - 0.5*(y_min_pix_objects[index]
                        + y_max_pix_objects[index]))
        if sky_background == "star" and do_targ is True:
            sky_pix_x = sky_pix_x + change_pix_x
            sky_pix_y = sky_pix_y + change_pix_y
        if do_targ:
            init_pix_x = 0.5*(x_min_pix_objects[index]
                              + x_max_pix_objects[index])
            init_pix_y = 0.5*(y_min_pix_objects[index]
                              + y_max_pix_objects[index])
        if do_comp:
            comp_pix_x = 0.5*(x_min_pix_objects[index_comp]
                              + x_max_pix_objects[index_comp])
            comp_pix_y = 0.5*(y_min_pix_objects[index_comp]
                              + y_max_pix_objects[index_comp])

    plt.close('all')

    print("        ... DONE!")
    print("\n     > Length of observing block is"
          " {:.4f} (hrs)".format(i*(exp_time+read_time)*24.))
...
if ext_coeff:
    print("\n (*) Including extinction correction using user-defined"
          " coefficient of: {}".format(ext_coeff))
...
diff_mag_err = [np.sqrt((a**2.)+(b**2.)) for a, b in zip(targ_mag_err,
                                                         comp_mag_err)]
...
datafile_id = open(out_dir + str(star_name) + "_" + str(image_filename)
                   + "_" + str(observatory) + ".dat", 'wb')
if ext_coeff:
    ...
    header = "BJD-2450000.0_(TDB)   diff_mag   diff_mag_err   target_mag" \
             ...
             "   target_airmass"
    np.savetxt(datafile_id, data.T, fmt='%.8f', delimiter="   ", header=header)
else:
    data = np.array([final_time_BJD, diff_mag, diff_mag_err, targ_mag,
                    targ_mag_err, comp_mag, comp_mag_err, final_time_HJD,
...
if do_plot:
    ...
    plt.savefig(out_dir + str(star_name) + "_" + str(image_filename)
                + "_" + str(observatory) + ".eps", dpi=300)
    plt.close(6)

if out_dir:
    print("\n (*) All (figures and ascii) files have been saved in"
          " {}".format(out_dir))
